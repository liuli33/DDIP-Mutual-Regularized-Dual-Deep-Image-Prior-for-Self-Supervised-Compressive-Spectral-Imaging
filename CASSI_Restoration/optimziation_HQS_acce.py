...
def ADMM_Iter(meas, Phi, data_truth, args, result_dir):
    ...
    im_input = get_input([1, 28, 256, 256]).to(device)
    im_input1 = get_input([1, 28, 256, 256]).to(device)
    im_net = Model(in_channels=28).cuda()
    im_net1 = Model(in_channels=28).cuda()
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, 10000, gamma=0.5, last_epoch=-1)
    scheduler_1 = torch.optim.lr_scheduler.StepLR(optimizer1, 10000, gamma=0.5, last_epoch=-1)
    ...
    for iter in range(iter_num):
        ...
        if iter < 500:
            ...
            loss = loss_fn(pred_response, meas)
            loss += loss_fn(for_lossx, model_out)
            loss1 = loss_fn(pred_response1, meas)
