...
class NonLocalMSA(nn.Module):
    ...
    def __init__(self,
        ...
        self.pos_emb = nn.Parameter(
            torch.Tensor(1, num_heads, window_num[0] * window_num[1], window_num[0] * window_num[1]))
        trunc_normal_(self.pos_emb)
    ...
    def forward(self, x):
        ...
        out = rearrange(out, 'b h n d -> b n (h d)')
        out = rearrange(out, 'b (h w) (b0 b1 c) -> (b h w) (b0 b1) c', h=self.window_num[0], b0=h // self.window_num[0],
                        b1=w // self.window_num[1])
...
class Model_Down(nn.Module):
    ...
    def __init__(self, in_channels, nd=128, kd=3, padding=1, stride=2):
        ...
        self.down_rb = SpaBlock(nd,1, (16,16),(16,16),"Gated_Dconv_FeedForward",1,)
        # self.down_rb1 = SpaBlock(nd,1, (8,8),(8,8),"Gated_Dconv_FeedForward",1,)
        # self.down_rb2 = SpaBlock(nd,1, (8,8),(8,8),"Gated_Dconv_FeedForward",1,)
        # self.down_rb3 = SpaBlock(nd,1, (8,8),(8,8),"Gated_Dconv_FeedForward",1,)
        self.relu2 = nn.ELU()
        # self.relu3 = nn.ELU()
        # self.relu4 = nn.ELU()
...
class Model_Skip(nn.Module):
    ...
    def __init__(self, in_channels=128, ns=4, ks=1, padding=0, stride=1):
        ...
        self.relu = nn.ELU()
        self.sk_rb = SpaBlock(ns,1, (16,16),(16,16),"Gated_Dconv_FeedForward",1,)
...
class Model_Up(nn.Module):
    ...
    def __init__(self, in_channels=132, nu=128, ku=3, padding=1):
        ...
        self.up_rb = SpaBlock(nu,1, (16,16),(16,16),"Gated_Dconv_FeedForward",1,)
        self.up_rb1 = SpaBlock(nu,1, (16,16),(16,16),"Gated_Dconv_FeedForward",1,)
        self.relu = nn.ELU()
        self.relu2 = nn.ELU()
        self.relu3 = nn.ELU()
        self.relu4 = nn.ELU()
        # self.up_rb1 = SpaBlock(nu,1, (8,8),(8,8),"Gated_Dconv_FeedForward",1,)
