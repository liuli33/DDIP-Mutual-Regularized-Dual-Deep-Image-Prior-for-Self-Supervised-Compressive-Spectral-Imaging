...
def shift_back(inputs, step):
    ...
    [h, w, nC] = inputs1.shape
    for i in range(nC):
        inputs1[:, :, i] = torch.roll(inputs1[:, :, i], (-1)*step*i, dims=1)
    output = inputs1[:, 0 : w - step*(nC - 1), :]
    return output
...
def calculate_ssim(data, recon, border=0):
    ...
    if data.ndim == 2:
        ...
        return ssim_(torch.unsqueeze(data, 0).permute(3, 0, 1, 2), torch.unsqueeze(recon, 0).permute(3, 0, 1, 2), data_range=1).data
...
def TV_denoiser(x, _lambda, n_iter_max):
    ...
    il[0] = 0
    p1 = 0.48*(torch.zeros_like(x))
    p2 = 0.48*(torch.zeros_like(x))
    p3 = 0.48*(torch.zeros_like(x))
    p4 = 0.48*(torch.zeros_like(x))
    ...
    for i in range(n_iter_max):
        ...
        divp = p1-p1[:,il,:] + p2 - p2[iux,:,:] + p3[idx,:,:] - p3 + p4[:,ir,:] - p4
    u = x - divp/_lambda
    return u


def TV_minimization(x, _lambda, n_iter_max):
    dt = 0.25
    N = x.shape
    idx = torch.arange(1, N[0]+1)
    idx[-1] = N[0]-1
...
# def get_input(tensize, const=100.0):
